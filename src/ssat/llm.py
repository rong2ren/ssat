...
class LLMClient:
    ...
    def call_llm(
        ...
        if provider == LLMProvider.OPENAI:
            return self._call_openai(system_message, prompt, temperature, max_tokens, max_retries, retry_delay)
        elif provider == LLMProvider.GEMINI:
            return self._call_gemini(system_message, prompt, temperature, max_tokens, max_retries, retry_delay)
        elif provider == LLMProvider.DEEPSEEK:
            return self._call_deepseek(system_message, prompt, temperature, max_tokens, max_retries, retry_delay)
        else:
            raise ValueError(f"Provider {provider.value} not implemented")
    
    def _call_openai(self, system_message: str, prompt: str, temperature: float = 0.4, 
                    max_tokens: int = 2000, max_retries: int = 3, retry_delay: float = 1.0) -> Optional[str]:
        """Call OpenAI API."""
        import openai
    ...
    def _call_gemini(self, system_message: str, prompt: str, temperature: float = 0.4, 
                    max_tokens: int = 2000, max_retries: int = 3, retry_delay: float = 1.0) -> Optional[str]:
    ...
    def _call_deepseek(self, system_message: str, prompt: str, temperature: float = 0.4, 
                      max_tokens: int = 2000, max_retries: int = 3, retry_delay: float = 1.0) -> Optional[str]:
...
def call_openai_chat(system_message: str, prompt: str, **kwargs) -> Optional[str]:
    """Legacy function for backward compatibility."""
    return llm_client.call_llm(LLMProvider.OPENAI, system_message, prompt, **kwargs)
